...
public class CintaCorredera : MonoBehaviour
{
    ...
    private void OnCollisionStay2D(Collision2D collision)
    {
        if (collision.gameObject.GetComponent<MightyLifeComponent>() != null && collision.gameObject.GetComponent<CharacterController>().GetIsGrounded() &&
            (collision.gameObject.GetComponent<InputComponent>().enabled == true || (collision.gameObject.GetComponent<InputComponent>().enabled == false && collision.gameObject.GetComponent<CharacterController>().GetDash())))
        {
